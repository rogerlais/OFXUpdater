...
public class BBOFXStringConversation extends DefaultStringConversion {
    ...
    protected String formatTime(Time time) {
        ...
        String result = String.format("%1$tH%1$tM%1$tS[%Z:%z]", calendar);
        return result;
    ...
    protected String formatDate(Date date) {
        // todo formatar de acordo com a regr ado BB
        GregorianCalendar calendar = new GregorianCalendar(TimeZone.getTimeZone("BRT"));
        calendar.setTime(date);
        String result = String.format("%1$tY%1$tm%1$td%1$tH%1$tM%1$tS[%Z:%z]", calendar);
        return result;
    }
